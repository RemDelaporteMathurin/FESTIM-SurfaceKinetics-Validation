...
T_stop_D = [420, 490, 610, 690, 750] # temperature at which the D irradiation was stopped, K
...
for i in range(len(T_stop_D)):
    ...
    t_cool.append((T_exposure[i]- T_storage) / cooling_rate[i]) # cooling time after sequential exposure, K
...
for i in range(1):
    ...
    def J_vs(T, surf_conc, t):
        ...
        cond2 = f.conditional(t >  t_exposure+t_cool[i]+t_storage, 
                f.conditional(t <= t_exposure+t_cool[i]+t_storage+t_add+dt_stop_D, 1, 0), 0)
    ...
    damaged_density1 = Wdam_trap1_conc_max[i]*rho_W*distr * sp.Piecewise((1 - sp.exp(- (F.t*phi_W/Phi_i_W1[i])), F.t <= t_exposure),
                                                                         (1 - sp.exp(- (t_exposure*phi_W/Phi_i_W1[i])), True)
                                                                         )
    damaged_density2 = Wdam_trap2_conc_max[i]*rho_W*distr * sp.Piecewise((1 - sp.exp(- (F.t*phi_W/Phi_i_W2[i])), F.t <= t_exposure),
    ...
    W_model.T = F.Temperature(
        value=sp.Piecewise(
            ...
            (T_exposure[i] - cooling_rate[i] * (F.t - t_exposure), F.t <= t_exposure + t_cool[i]),
            (T_storage, F.t <= t_exposure + t_cool[i] + t_storage),     
            (T_add, F.t <= t_exposure + t_cool[i] + t_storage + t_add),
            (T_add - cooling_rate[1] * (F.t - t_exposure - t_cool[i] - t_storage  - t_add), F.t <= t_exposure + t_cool[i] + t_storage + t_add + t_cool[1]),
            (T_storage, F.t <= t_exposure + t_cool[i] + 2*t_storage + t_add + t_cool[1]),
            (T_storage + ramp * (F.t - t_exposure - t_cool[i] - 2*t_storage - t_add - t_cool[1]), True),
        )
    )
    ...
    def dt(t):
        if t < t_exposure:
            return 100
        elif t > t_exposure + t_cool[i] + t_storage - 50 and t < t_exposure + t_cool[i] + t_storage + t_add:
            return 100
    ...
    W_model.settings = F.Settings(
        ...
        final_time=t_exposure+ t_cool[i] + 2*t_storage + t_heat_add + t_add + t_cool[1] + t_TDS,
    )
    ...
    plt.plot(T_storage + ramp * (results["t(s)"]-(t_exposure + t_cool[i] + 2*t_storage + t_cool[1] + t_heat_add + t_add)), 
             results["Flux1"],
